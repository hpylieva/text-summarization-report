...
Then a function $g: R^H \rightarrow R^V$ is applied so that $s_t := g(s_{t-1}, [v, y_{t-1}]) \in R^V$ is a vector of the same size as vocabulary ($s_0 := g([v, <eos>]$). Then we predict next word $y_t$ in output sequence by applying softmax to $s_j$. \todo{Does this sentence easy to understand?} Softmax function results in a vector of likelihood for each word in the vocabulary to be the next in the sequence. 
...
With attention mechanism the decoder relies not only on a single hidden state generated by encoder, but has information about hidden states ${h_i}$ generated by encoder on each time stamp. As shown on Figure~\ref{fig:attention} encoder's hidden states are weighted.  To make 

\begin{figure}[h]
\centering
	\includegraphics[width=0.5\textwidth]{img/seq2seq-with-attention.png}
	\setcaptioncitation{\url{https://machinelearningmastery.com/global-attention-for-encoder-decoder-recurrent-neural-networks/}}
	\caption{\label{fig:attention} }
\end{figure}
...
\section{Dataset}
\subsection{Overview}
\todo[inline, color=green!40]{Where the dataset is from. What it contains. }

\subsection{Preprocessing}
\todo[inline, color=green!40]{Start with reformating that strange JSON that theyy had. Then, all the stuff we do in vocabulary-embedding part}
